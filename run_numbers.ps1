...
function Invoke-Meas{
   ...
   $cl_command = "CL " + $include_statement + "/O2 /GL /Oi /MD /D NDEBUG /D " + "i_" + $inc + " " + $extra_defines_str + " /std:c++latest /experimental:module /EHsc /nologo build_project/main.cpp build_project/tu0.cpp build_project/tu1.cpp build_project/tu2.cpp build_project/tu3.cpp build_project/tu4.cpp /link /MACHINE:X64 /LTCG:incremental"
   Write-Host ("description: {0}, inc: {1}, defines: {2}" -f $description, $inc, [system.String]::Join(" ", $defines)) -ForegroundColor DarkGreen
   ...
   For ($i=0; $i -lt $repeats; $i++){
      if($description -eq "warmup"){
         ...
         } | Out-File -FilePath ("measurements\$description-$inc-" + $defines[0] +".txt") -Append -Encoding utf8
      }
...
$std_modules = "std_regex","std_filesystem"
...
Invoke-Meas -description "warmup" -inc "warmup" -repeats $repetitions -defines @()
Invoke-Meas -description "special" -inc "baseline" -repeats $repetitions -defines @("no_std")
Invoke-Meas -description "special" -inc "baseline" -repeats $repetitions -defines @("all_std")

# Foreach($header in $third_party_libs){
#    Invoke-Meas -description "third_party" -inc $header -repeats $repetitions -defines @("no_std")
#    Invoke-Meas -description "third_party" -inc $header -repeats $repetitions -defines @("all_std")
# }

# Foreach($header in $boost_headers){
#    Invoke-Meas -description "boost" -inc $header -repeats $repetitions -defines @("no_std")
#    Invoke-Meas -description "boost" -inc $header -repeats $repetitions -defines @("all_std")
# }


Foreach($header in $std_headers){
   Invoke-Meas -description "std" -inc $header -repeats $repetitions -defines @("no_std")

   $def = "no_" + $header
   Invoke-Meas -description "std" -inc $header -repeats $repetitions -defines @($def, "all_std")
}

# Foreach($header in $std_modules){
#    Invoke-Meas -description "std_modules" -inc $header -repeats $repetitions -defines @("no_std")
# }
